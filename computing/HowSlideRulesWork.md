...
***precision***—Before we discuss precision of slide rules, let us explore the nature of the logarithmic scale. The $C$ and $D$ scales are logarithmic, so they are nonlinear. The scales start on the left at $log(1) = 0$, which is marked as $1$, and end on the right at $log(10) = 1$, which is also marked as $1$. Indeed, these scales wrap around by multiples of $10$ and, hence, the $1$ mark at both ends. So, the scale reading of $314$ can be valued as $0.0314, 0.314, 3.14,$ etc., depending on the context. The decimal point must be located using mental arithmetic. For example, $\pi/8 \approx 3/8 \approx 0.4$, so the result must necessarily be $0.3927$, not $0.03927, 3.927,$ or any other alternative. To put it another way, the mental arithmetic gives us the decimal point location, and the scale reading gives us the constituent digits.
