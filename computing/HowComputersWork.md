...
```
       ...
       if a < b jump HERE, else jump THERE
       ...
HERE:  here code block
       ...
THERE: there code block
       ...
```

The above conditional branch is computed by comparing two integers `a` and `b`. The CPU compares these two integers by first computing the subtraction `a - b`, then checking if the result is negative, zero, or otherwise. A negative means `a < b`; a zero means `a = b`; otherwise, `a > b`. By taking one execution path or another on the basis of this result, the CPU performs conditional branching.

Repetitive execution of a block of code, known as looping, is accomplished by jumping back to the top of the block, based on a branching condition located at the bottom of the block:

```
      ...
LOOP: i = 0
      ...
      if i < n jump LOOP, else jump OUT
OUT:  ...
```
...
Algorithms like these are implemented as functions that take inputs and return results. When performing a conditional branch, the CPU can jump immediately to the designated code blocks, depending on the condition. But when calling a function, the CPU must assemble the input values for the function to use, and also provide a way for the function to return its result.

Once simple algorithms have been implemented on a CPU, more sophisticated ones like [sorting](https://en.wikipedia.org/wiki/Sorting_algorithm), [searching](https://en.wikipedia.org/wiki/Search_algorithm), and [graph](https://en.wikipedia.org/wiki/Graph_theory) algorithms can be implemented by composing simpler ones. Complicated AI algorithms like [chess](https://en.wikipedia.org/wiki/Computer_chess) and [Go](https://en.wikipedia.org/wiki/Computer_Go) quickly follow. From a stream of `0`s and `1`s thus arose semblance of intelligence.
