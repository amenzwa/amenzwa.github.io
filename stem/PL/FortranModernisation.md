...
Not all is lost, however. A dependent type system does not demand blind obedience from the programmer: not all types need be adorned with fully-dependent regalia. That is, the programmer selects the desired level of type specificity. For example, we may type `qsort` with a progressively finer specificity, as follows:
