...
- A polymorphic type, like `'a list`, corresponds to universal quantification $∀\alpha$. Also, a [dependent product type](https://en.wikipedia.org/wiki/Dependent_type#%CE%A0_type) $\Pi$ is universal quantification $\forall$. And a [dependent sum type](https://en.wikipedia.org/wiki/Dependent_type#%CE%A3_type) $\Sigma$ is existential quantification $\exists$. ($\Pi$ and $\Sigma$ are concepts of the dependent type theory discussed in more detail in the type theory section of my article [*Computer Science Curriculum*](../CS/Curriculum.md)).
