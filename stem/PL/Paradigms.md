...
The *delayed evaluation* strategy looks similar to lazy evaluation, but it is very different. Modern languages do not use it, but modern applications do. That is, it is not a language feature but a programming technique. It is known in the web application world as "future". A delayed evaluation is but a suspension of computation for a future evaluation. When the suspended computation is eventually resumed, the whole expression is reduced to its values, in one go. Delayed evaluation is often used to postpone expensive computations until the point when there is time to spare. Large media files and large JavaScript modules embedded in web pages are often loaded using application code that employs the delayed evaluation strategy. Delayed evaluation, by definition, does not memoise the results. Every access to the expression recomputes it. Of course, the programmer may, on his own, augment his delayed evaluation code with memoisation.
...
The *call-by-name* parameter passing strategy, by far the most popular with language designers, reduces the arguments to their values before passing them into the function. Eager languages use this approach. It is natural.

The *call-by-reference* strategy is an optimisation technique. Instead of passing, by value, the entire copies of large objects, the language passes only the pointers to those objects, and coats the bitter pointers with syntactic sugar, so the programmer need not grapple with pointer dereferencing.

The *call-by-name* strategy was used in ALGOL 60. It is not popular with modern language designers. Under the call-by-name strategy, the language passes the arguments, unevaluated, into the function. The function receives just names (references) to the arguments. Only when the function accesses the argument, its expression is reduced, all in one go, to its value, completely. The call-by-name parameter passing is employed by languages that use the delayed evaluation strategy.

The *call-by-need* strategy is used in Haskell and its lazy siblings. The language passes argument expressions into to the function without evaluation. Only when the function accesses the argument, its expression is reduced, and even then, only incrementally up to the values needed.
