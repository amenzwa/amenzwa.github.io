...
Most modern programming language, be they OO or FP, support an incarnation of the Hindley-Milner type system. So, today's programmers are generally familiar with the concepts of this type system. But only those who know the proof-assistant hereditary of the original version of ML, the LCF/ML, recognise that types are theorems, functions on those types are inference rules, functionals are proof tactics, and type system was there to guarantee the logical validity of theorems. A tactic is a backward proof procedure that starts with the goal, the proposition to be proved, and works backward to reach proven sub-propositions (lemmas).
