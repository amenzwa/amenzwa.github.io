...
- A type (a set, by analogy) is a proposition. A typed expression's value (an element) is a proof of the proposition that corresponds to that type. An evaluation of an expression is simplification of a proof. The type system guarantees the valid typing of the expressions and, as such, guarantees the logical validity of the corresponding proofs.
- Constructing a value of a type (an element of a set) is proving the proposition that corresponds to that type. An inhabited type (a non-empty set) is a proven proposition, which is either a lemma or a theorem. An uninhabited type (the empty set $\empty$), is a logical [contradiction](https://en.wikipedia.org/wiki/Contradiction) $\bot$.
- A product type (a pair) is a logical conjunction. A sum type (a disjoint union) is a logical disjunction. The nullary product type (the `Unit` type) is the logical $True$ value. The nullary sum type (the `Void` type) is the logical $False$ value. A function from some type to the `Void` type is a logical negation and an absurdity $\bot$.
- A function from one type to another (a map between sets) is an implication. Applying a function is employing [modus ponens](https://en.wikipedia.org/wiki/Modus_ponens). An identity function is reflexivity, a tautology. A bijective function is symmetry, an equivalence. A composition of functions is transitivity. A recursive function is an inductive proof. (Recall, from above, that a function is a product type, namely a set of pairs.)
