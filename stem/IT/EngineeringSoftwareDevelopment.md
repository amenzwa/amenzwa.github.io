...
In this article, I describe the large-scale software development processes and practices commonly used in IT for decades. First, let me clear the air. There is no such thing as a "large, real-time, embedded software"—at least it ought not exist—only large, real-time systems built out of many "small, embedded software" components that monitor and control several hardware components.
